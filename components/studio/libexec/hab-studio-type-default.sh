...
finish_setup() {
  ...
  # In the overwhelming majority of instances, you will want use
  # stable components in your Studio.
  #
  # However, the Habitat team needs to build studios using unstable
  # components in order to fully test things. This `OVERRIDE_CHANNEL`
  ...
  for pkg in $pkgs; do
    if [[ -n "${OVERRIDE_CHANNEL:-}" ]]; then
      info "Override channel set; retrieving ${pkg} from ${OVERRIDE_CHANNEL}"
      _hab install --channel="${OVERRIDE_CHANNEL}" ${pkg} || {
         info "Package not found in ${OVERRIDE_CHANNEL}; falling back to stable"
        _hab install ${pkg}
      }
    else
      _hab install $pkg
